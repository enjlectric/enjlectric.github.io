    ...
    render() {
        return (
            <div className="projectPage">
                ...
                <p>
                    Barry's Bog Jog is a <span>top-down dungeon crawling game</span>, inspired by SPIKE CHUNSOFT's Mystery Dungeon games. It was created in 7 days for the 2022 7DRL (7 Day Roguelike) Jam and features 3 areas with 29 floors total.
                </p>
                <h2>
                    Game Mechanics
                </h2>
                ...
                <p>
                    <span>Player input</span> is simply the player's turn. It waits for the player to select an action, then registers it as the player's action for this turn. If the action is movement, a special flag is set that keeps the doors open for all movement to happen in parallel (assuming no enemy attacks the player at their new position).
                </p>
                <p>
                    <span>Enemy thinking</span> uses the player's scheduled new coordinate in the thought processing, to ensure that enemies don't lag behind a turn. Extra care is put into avoiding two enemies claiming the same coordinate to move to.
                </p>
                <p>
                    During <span>execution</span>, first, the player's turn is executed. Then, enemy turns are executed. If all enemy turns/all turns including the player turn are movement turns, all turns happen in parallel. It is also possible for enemy and player positions to change unexpectedly during the turn execution, in which case the aforementioned rough spots take effect. I have since conceptualized an upgrade to this system for future mystery dungeon titles, where an enemy's thought can be dirtied based on whether its target cell state changes between thinking of and executing a command.
                </p>
                <h2>
                    Behind the scenes
                </h2>
                ...
                <span>Barry's Bog Jog is available for free on: <a target="_blank" rel='noreferrer' href="https://enjl.itch.io/barrys-bog-jog">Itch.io</a></span>
            </div>
        )
    }
}
